...
pub fn createStep(b: *std.Build, name: []const u8, options: CompileCommandOptions) void {
    const custom_step = b.allocator.create(CcdbStep) catch
        @panic("Allocation failure, probably OOM");
                ...
                .other_step => |compile| {
                    for (compile.installed_headers.items) |install_header| {
                        switch (install_header) {
                            .file => {},
                            .directory => |dir| {
                                dir.source.addStepDependencies(&custom_step.step);
                            },
                        }
                    }
                    if (options.include_emitted_include_trees) {
                        _ = compile.getEmittedIncludeTree();
                        custom_step.step.dependOn(&compile.installed_headers_include_tree.?.step);
                    }
                },
                .path, .path_system, .path_after, .framework_path, .framework_path_system => |path| {
                    path.addStepDependencies(&custom_step.step);
                },
        ...
        for (compile_step.root_module.link_objects.items) |link_object| {
            switch (link_object) {
                ...
                .c_source_file => |c_source_file| {
                    ...
                    c_source_file.file.addStepDependencies(&custom_step.step);
                },
                .c_source_files => |c_source_files| {
                    c_source_files.root.addStepDependencies(&custom_step.step);
                },
            }
        }
        index += 1;
    ...
    cdb_step.dependOn(&custom_step.step);
}
            ...
            switch (link_object) {
                ...
                .c_source_file => {
                    ...
                    source_file.* = CSourceFiles{
                        .root = b.path("."),
                        .language = link_object.c_source_file.language,
                        .files = files_mem,
...
fn makeCdb(step: *std.Build.Step, opts: std.Build.Step.MakeOptions) anyerror!void {
    _ = opts;
    const self: *CcdbStep = @fieldParentPtr("step", step);
    ...
    const c_sources = getCSources(step.owner, self.options);

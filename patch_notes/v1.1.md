...
- Re-implemented some features from [DSDA Doom Dev Branch](/patch_notes/v0.28.md)
  ...
  - Minor OpenGL fixes (Pedro-Beirao)
...
- "Pistol Start" Menu Option
  ...
  - "-pistolstart" argument now sets config to "session". This means it's possible to turn the argument off in-game.
- Animated widescreen / background revamp
  ...
  - Added support for animated `STBAR` and `STARMS`.
- Added new option for **Armour** hud element *(Status Bar / HUD > Status Indicators)*
  - This option may be more compatible with custom statusbars.
